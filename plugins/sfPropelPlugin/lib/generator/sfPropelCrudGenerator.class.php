...
class sfPropelCrudGenerator extends sfAdminGenerator
{
  ...
  public function getCrudColumnEditTag($column,$mostrarValue=true, $params = array())
  {
    $type = $column->getType();
    ...
    if ($column->isForeignKey())
    {
      ...
      //return $this->getPHPObjectHelper('select_tag', $column, $params, array('related_class' => $this->getRelatedClassName($column)));
        $objeto = "\n";
        $objeto .= "this.".$column->getName()." = new Ext.form.ComboBox({\n";
        $objeto .= "\tfieldLabel:'".$this->getParameterValue("editar.fields.".$column->getName().".name")."',\n";
        $objeto .= "\tstore: this.store".$column->getRelatedColumnName().",\n";
        $objeto .= "\ttypeAhead: true,\n";
        $objeto .= "\tvalueField: '".strtolower($column->getRelatedColumnName())."',\n";
        $objeto .= "\tdisplayField:'".strtolower($column->getRelatedColumnName())."',\n";

        if($mostrarValue==true){
        $objeto .= "\thiddenName:'".$this->getSingularName()."[".$column->getName()."]',\n";
        }else{
        $objeto .= "\thiddenName:'".$column->getName()."',\n";
        }
        
        $objeto .= "\t//readOnly:(this.OBJ.".$column->getName()."!='')?true:false,\n";
        $objeto .= "\t//style:(this.main.OBJ.".$column->getName()."!='')?'background:#c9c9c9;':'',\n";

        $objeto .= "\tforceSelection:true,\n";
        //        $objeto .= "name: '".$column->getName()."x',\n";
        //        $objeto .= "id: 'id_".$column->getName()."x',\n";
        $objeto .= "\tresizable:true,\n";
        $objeto .= "\ttriggerAction: 'all',\n";
        $objeto .= "\temptyText:'Seleccione ".$column->getName()."',\n";
        $objeto .= "\tselectOnFocus: true,\n";
        $objeto .= "\tmode: 'local',\n";
        $objeto .= "\twidth:200,\n";
        $objeto .= "\tresizable:true,\n";
        $objeto .= "\tallowBlank:false\n";
        $objeto .= "});\n";

        $objeto .= "this.store".$column->getRelatedColumnName().".load();\n";
        if($mostrarValue==true){
//        $objeto .= "this.".$column->getName().".setValue('Cargando..');\n";
//            $objeto .= "this.store".$column->getRelatedColumnName().".load({\n";
//                $objeto .= "callback:function(){\n";
//                    $objeto .= "//if(this.OBJ.".$column->getName()."==''){this.".$column->getName().".setValue(\"\");}\n";
//                $objeto .= "}\n";
//            $objeto .= "});\n";
            $objeto .= "\tpaqueteComunJS.funcion.seleccionarComboByCo({\n";
                $objeto .= "\tobjCMB: this.".$column->getName().",\n";
                $objeto .= "\tvalue:  this.OBJ.".$column->getName().",\n";
                $objeto .= "\tobjStore: this.store".$column->getRelatedColumnName()."\n";
            $objeto .= "});\n";
    ...
    else if ($type == PropelColumnTypes::DATE)
    {
      ...
      $objeto = "\n";
      $objeto .= "this.".$column->getName()." = new Ext.form.DateField({\n";
      $objeto .= "\tfieldLabel:'".$this->getParameterValue("editar.fields.".$column->getName().".name")."',\n";
      if($mostrarValue==true){
            $objeto .= "\tname:'".$this->getSingularName()."[".$column->getName()."]',\n";
            $objeto .= "\tvalue:this.OBJ.".$column->getName().",\n";
            $objeto .= "\tallowBlank:false,\n";
	    $objeto .= "\twidth:100\n";	
      }else{//para cuando toca el filtro, ya que no puede llevar la nomesclatura de: tabla[campo], si no: campo.
            $objeto .= "\tname:'".$column->getName()."'\n";
      }
      $objeto .= "});\n";
      return $objeto;
//      return $this->getPHPObjectHelper('input_date_tag', $column, $params, array('rich' => true));
    ...
    {
      $objeto = "\n";
      $objeto .= "this.".$column->getName()." = new Ext.form.Checkbox({\n";
        $objeto .= "\tfieldLabel:'".$this->getParameterValue("editar.fields.".$column->getName().".name")."',\n";
        if($mostrarValue==true){
            $objeto .= "\tname:'".$this->getSingularName()."[".$column->getName()."]',\n";
            $objeto .= "\tchecked:(this.OBJ.".$column->getName()."=='0') ? true:false,\n";
            $objeto .= "\tallowBlank:false\n";
        }else {
            $objeto .= "\tname:'".$column->getName()."',\n";
            $objeto .= "\tchecked:true\n";
...
//      $size = ($column->getSize() > 20 ? ($column->getSize() < 80 ? $column->getSize() : 80) : 20);
      $objeto = "\n";
      if($column->getName()=='in_delete'){
        $objeto .= "this.".$column->getName()." = new Ext.form.Checkbox({\n";
      }else{
        $objeto .= "this.".$column->getName()." = new Ext.form.TextField({\n";
      }
      ...
      if($column->getName()=='in_delete') {
          if($mostrarValue==true){
                $objeto .= "\tname:'".$this->getSingularName()."[".$column->getName()."]',\n";
                $objeto .= "\tchecked:(this.OBJ.in_delete=='0') ? true:false,\n";
                $objeto .= "\tallowBlank:false\n";
          }else {
                $objeto .= "\tname:'".$column->getName()."',\n";
                $objeto .= "\tchecked:true\n";
...
//      $size = ($column->getSize() > 20 ? ($column->getSize() < 80 ? $column->getSize() : 80) : 20);
      ...
      if($column->getName()=='in_delete'){
        ...
        $objeto .= "this.".$column->getName()." = new Ext.form.TextArea({\n";
      }
...
//    return $this->getPHPObjectHelper('input_tag', $column, $params, array('size' => $size));
    ...
    else if ($type == PropelColumnTypes::INTEGER || $type == PropelColumnTypes::TINYINT || $type == PropelColumnTypes::SMALLINT || $type == PropelColumnTypes::BIGINT)
    {
      ...
      if($column->getName()=='in_delete'){
        ...
        $objeto .= "this.".$column->getName()." = new Ext.form.NumberField({\n";
      }
   ...
   * @return string HTML code
   */
  public function getColumnEditTag($column,$mostrarValue = true, $params = array())
  {
    ...
    return $this->getCrudColumnEditTag($column,$mostrarValue, $params);
  }
